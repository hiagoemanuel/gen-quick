...
export const Menu = () => {
  const { isOpen, setIsOpen, historyOpen, setHistoryOpen } = useContext(MenuContext)
      ...
      {isOpen && (
        ...
        >
          <motion.div
            ...
            animate={{
              y: 0,
              x: historyOpen ? '-100vw' : 0,
              transition: { delay: 0.25, type: 'just' },
            }}
            exit={{ y: '50vh' }}
          ...
          <MenuHistory isOpen={historyOpen} />
        </motion.section>
      )}
    </AnimatePresence>
  )
